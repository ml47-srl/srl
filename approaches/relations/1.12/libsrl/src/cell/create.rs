...
pub fn simple_by_tokens(tokens : Vec<String>) -> Result<Cell, ()> {
	if ! is_valid_id(&tokens[0]) {
		panic!("simple_by_tokens(): invalid id");
	}
	...
	while ! tokens.is_empty() {
		...
		if ! is_valid_id(&token) {
			if token == "(" {
				...
				panic!("complex_by_tokens(): weird invalid token='{}'", token);
			}
		}
		if parens == 0 {
			match cell_by_tokens(tmp_tokens) {
				...
				_ => panic!("complex_by_tokens(): recursive call failed")
			}
			tmp_tokens = Vec::new();
		}
	}
...
pub fn cell_by_tokens(mut tokens : Vec<String>) -> Result<Cell, ()> {
	...
	if len == 0 {
		...
		return simple_by_tokens(tokens);
	} else if tokens[0] == "{" && tokens[len-1] == "}" {
		return scope_by_tokens(tokens);
	} else if tokens[0] == "[" && tokens[len-1] == "]" {
		return var_by_tokens(tokens);
	} else {
		return complex_by_tokens(tokens);
	}
}
