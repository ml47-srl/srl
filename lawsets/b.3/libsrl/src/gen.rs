...
pub fn true_cell() -> Cell {
	simple_by_str("'true'").unwrap()
}

pub fn false_cell() -> Cell {
	simple_by_str("'false'").unwrap()
}

pub fn equals_cell(cell1 : Cell, cell2 : Cell) -> Cell {
	complex(vec![simple_by_str("=").unwrap(), cell1, cell2])
}

pub fn simple(string_arg : String) -> Result<Cell, SRLError> {
	Ok(Cell::Simple { string : SimpleString::create(string_arg)? })
}

pub fn simple_by_str(string_arg : &str) -> Result<Cell, SRLError> {
	Ok(Cell::Simple { string : SimpleString::create(string_arg.to_string())? })
}
